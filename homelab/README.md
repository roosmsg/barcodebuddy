...
(Watchtower keeps it updated). The changes in this fork are applied to that image
at runtime instead of building our own image:

| Change in this fork | How it reaches the running container |
| --- | --- |
...
`/opt/barcodebuddy/`). `master` carries these changes on top of upstream; sync by
merging `upstream/master` into it.
